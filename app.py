...
with st.container():
    ...
    with columns[0]:
        ...
        average_cell_area = st.slider("Average Size of Single Cell", min_value=200, max_value=800, value=average_cell_area, step=10)
        connected_cell_area = st.slider(
            "Max Size of Cell (for cases where cells are stuck together)", min_value=100, max_value=2000, value=connected_cell_area, step=10)
    with columns[1]:
